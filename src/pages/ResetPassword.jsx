    ...
    <div style={{ minHeight:'100vh', background:'#0a0c0f', fontFamily:'"DM Sans", system-ui, sans-serif', color:'#e8eaf0', display:'flex', overflow:'hidden' }}>
      <style>{`
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .rp-input { width:100%; padding:13px 16px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); border-radius:3px; color:#e8eaf0; font-size:14px; font-family:"DM Sans",sans-serif; font-weight:300; outline:none; transition:border-color 0.2s, box-shadow 0.2s; }
        .rp-input::placeholder { color:rgba(232,234,240,0.2); }
        .rp-input:focus { border-color:rgba(200,245,225,0.35); box-shadow:0 0 0 3px rgba(200,245,225,0.06); }
        .rp-input.mismatch { border-color:rgba(251,113,133,0.4); box-shadow:0 0 0 3px rgba(251,113,133,0.05); }
        .rp-input.match { border-color:rgba(200,245,225,0.3); box-shadow:0 0 0 3px rgba(200,245,225,0.05); }

        .rp-submit { width:100%; background:#e8eaf0; color:#0a0c0f; border:none; padding:14px; border-radius:3px; font-size:14px; font-weight:600; cursor:pointer; font-family:"DM Sans",sans-serif; transition:all 0.2s; letter-spacing:0.01em; }
        .rp-submit:hover:not(:disabled) { background:#fff; transform:translateY(-1px); }
        .rp-submit:disabled { opacity:0.35; cursor:not-allowed; }

        .rp-show-pass { position:absolute; right:14px; top:50%; transform:translateY(-50%); background:none; border:none; color:rgba(232,234,240,0.3); cursor:pointer; display:flex; align-items:center; transition:color 0.2s; padding:0; }
        .rp-show-pass:hover { color:rgba(232,234,240,0.6); }

        .rp-feature-row { display:flex; align-items:flex-start; gap:14px; padding:14px 0; border-bottom:1px solid rgba(255,255,255,0.05); }
        ...
        <div style={{ width:'100%', maxWidth:'380px', animation:'fadeUp 0.6s ease both' }}>
          ...
          <div style={{ marginBottom:'32px' }}>
            <h1 style={{ fontFamily:'"Fraunces", serif', fontSize:'clamp(28px, 4vw, 38px)', fontWeight:300, letterSpacing:'-0.03em', lineHeight:1.05, marginBottom:'10px' }}>
              Choose a new<br/>
              <span style={{ fontStyle:'italic', color:'rgba(232,234,240,0.35)' }}>password.</span>
            </h1>
            <p style={{ color:'rgba(232,234,240,0.3)', fontSize:'14px', lineHeight:1.65, fontWeight:300 }}>Make it strong and memorable.</p>
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'14px', marginBottom:'20px' }}>
            ...
            {/* New Password */}
            <div>
              <label style={{ fontSize:'11px', fontWeight:500, color:'rgba(232,234,240,0.3)', letterSpacing:'0.08em', textTransform:'uppercase', display:'block', marginBottom:'8px' }}>New password</label>
              <div style={{ position:'relative' }}>
                ...
                />
                <button className="rp-show-pass" onClick={() => setShowPass(!showPass)} tabIndex={-1}>
            ...
            <div style={{ animation:'fadeIn 0.25s ease both' }}>
              ...
              <div style={{ position:'relative' }}>
                <input
                  ...
                  style={{ paddingRight:'44px' }}
                />
                <button className="rp-show-pass" onClick={() => setShowConfirmPass(!showConfirmPass)} tabIndex={-1}>
          ...
          {/* Submit */}
          <button className="rp-submit" onClick={handle} disabled={loading} style={{ marginBottom:'20px' }}>
            {loading ? 'Updating…' : 'Update password →'}
          </button>

          <p style={{ textAlign:'center', fontSize:'13px', color:'rgba(232,234,240,0.25)', fontWeight:300 }}>
            Remembered it?{' '}
            <span onClick={() => navigate('/login')} style={{ color:'rgba(200,245,225,0.7)', fontWeight:500, cursor:'pointer' }}>Sign in</span>
          </p>
      ...
      <div style={{ flex:1, borderLeft:'1px solid rgba(255,255,255,0.05)', background:'#0d1016', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 48px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ width:'100%', maxWidth:'320px', position:'relative', zIndex:1 }}>
          {[
            ...
            <div key={i} className="rp-feature-row">
